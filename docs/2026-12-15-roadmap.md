...
**MCP priority:** Improved primitives — the roadmap names scaling to large tool
catalogs, and cheap revalidation of cached lists is a precondition for that. It
also builds directly on the cacheable list results the roadmap counts as
delivered.
...
adds the *validator* half: ETags for conditional requests, and the same
mechanism carried down to the individual tool definition. Each tool in
`tools/list` gets an opaque version identifier; clients echo it on `tools/call`;
...
* ETag support on cacheable results, enabling conditional requests ("give me
  this list only if it changed since version X") to complement the shipped
  `ttlMs` freshness hints.
...
* Coherent layering: list ETag/TTL governs when to re-fetch; per-tool versions
  govern call-time validity.
...
* How do ETags interact with `cacheScope: "public"`, where a shared intermediary
  may be serving the cached copy?
...
* Interaction with caching: language becomes a cache key (`Vary:
  ...
  must account for it explicitly. This is sharper now that `cacheScope:
  "public"` permits shared intermediaries to hold list results.
...
**Goal:** Remove the ambiguity introduced when client capabilities moved from
initialization to per-request `_meta`: servers no longer know, at `tools/list`
time, which capabilities the client will declare on later `tools/call` requests
— so they cannot deterministically decide which tools to expose or how those
tools should behave. (Tracked in
...
* Interaction with caching: capability-dependent lists make the capability set a
  cache key — how do list ETags and `ttlMs` account for it? Sharper under
  `cacheScope: "public"`, where an intermediary may serve a list filtered for a
  different client.
* Can a client under-declare and upgrade later, and what does that do to cached
  lists? This connects to the roadmap's progressive-discovery problem for large
  tool catalogs.
